...
type BqaEditStep =
  ...
  | { step: "delivery_content"; itemId: number; catId: number; deliveryType: string; isNewDelivery?: boolean }
  | { step: "delivery_add_content"; itemId: number; catId: number; deliveryType: string };
...
export function startBot(expressApp?: Application): TelegramBot {
  ...
  // ── Filet de sécurité Markdown ────────────────────────────────────────────
  // Si un texte (ex: nom/description d'article saisi par un admin) contient des
  // caractères Markdown non fermés (* _ ` [ ]), Telegram renvoie une erreur
  // "can't parse entities" et le message n'est pas envoyé. On intercepte cette
  // erreur et on renvoie automatiquement le message en texte brut (sans parse_mode).
  ...
  function stripParseMode<T extends Record<string, unknown> | undefined>(opts: T): T {
    if (!opts) return opts;
    const clone = { ...opts } as Record<string, unknown>;
    delete clone["parse_mode"];
    return clone as T;
  }
  for (const method of ["sendMessage", "sendPhoto", "sendDocument", "sendVideo", "sendAudio", "sendAnimation", "editMessageText", "editMessageCaption"] as const) {
    const original = (bot as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>)[method].bind(bot);
    (bot as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>)[method] = async (...args: unknown[]) => {
      try {
        return await original(...args);
      } catch (err) {
        if (!isParseEntitiesError(err)) throw err;
        // On cherche l'argument d'options contenant parse_mode (généralement le
        // dernier, mais sendPhoto/sendDocument/... peuvent avoir un fileOptions
        // après l'objet d'options).
        const optsIdx = args.findIndex(
          (a) => a && typeof a === "object" && "parse_mode" in (a as Record<string, unknown>),
        );
        if (optsIdx !== -1) {
          const retryArgs = [...args];
          retryArgs[optsIdx] = stripParseMode(args[optsIdx] as Record<string, unknown>);
  ...
  // Clavier affiché après l'ajout d'un élément lors de la création (multi-livraison)
  function deliveryMoreKb(catId: number, count: number, cancelCb: string): TelegramBot.InlineKeyboardMarkup {
    return { inline_keyboard: [
      [{ text: "➕ Ajouter un autre élément", callback_data: `bqa_dlv_more_${catId}` }],
      [{ text: `✅ Terminer (${count} élément${count > 1 ? "s" : ""})`, callback_data: `bqa_dlv_done_${catId}` }],
      [{ text: "❌ Annuler", callback_data: cancelCb }],
    ]};
  }

  // Clavier de choix du type lors de l'ajout d'un élément à un article existant
  function deliveryTypeAddKb(itemId: number): TelegramBot.InlineKeyboardMarkup {
    return { inline_keyboard: [
      [{ text: "📝 Texte/Message", callback_data: `bqa_dlvadd_text_${itemId}` }, { text: "🖼️ Photo", callback_data: `bqa_dlvadd_photo_${itemId}` }],
      [{ text: "🎥 Vidéo", callback_data: `bqa_dlvadd_video_${itemId}` }, { text: "📄 Document", callback_data: `bqa_dlvadd_doc_${itemId}` }],
      [{ text: "🎵 Audio", callback_data: `bqa_dlvadd_audio_${itemId}` }, { text: "🎞️ GIF/Animation", callback_data: `bqa_dlvadd_anim_${itemId}` }],
    ...
    try {
      ...
      if (data.startsWith("bq_buy_cnf_")) {
        ...
        if (hasAutoDelivery) {
          if (itemDeliveries.length > 0) {
            ...
            await sendSingleDelivery(chatId, { id: 0, itemId: item.id, type: item.deliveryType, fileId: item.deliveryFileId ?? null, content: item.deliveryCaption ?? null, createdAt: new Date() });
          }
        }
      ...
      if (data.startsWith("bqa_") && isAdmin(userId)) {
        ...
        if (data.startsWith("bqa_item_") && !data.startsWith("bqa_item_cnf_")) {
          ...
          if (!item) return;
          const price = parseFloat(item.price);
          const deliveryLabels: Record<string, string> = { text: "📝 Texte", photo: "🖼️ Photo", video: "🎥 Vidéo", document: "📄 Document", audio: "🎵 Audio", animation: "🎞️ GIF" };
          const deliveryLabel = item.deliveryType
            ? deliveryLabels[item.deliveryType] ?? item.deliveryType
            : "❌ Aucune (manuelle)";
          ...
          // ── Boutique Admin : gestion des livraisons d'un article ───────────
          if (data.startsWith("bqa_edit_delivery_") && isAdmin(userId)) {
            ...
            if (!item) return;
            const emojiMap: Record<string, string> = { text: "📝", photo: "🖼️", video: "🎥", document: "📄", audio: "🎵", animation: "🎞️" };
            const dlvLines = deliveries.map((d, i) =>
              `${i + 1}. ${emojiMap[d.type] ?? "❓"} *${d.type}*${d.content ? ` — "${d.content.slice(0, 40)}${d.content.length > 40 ? "…" : ""}"` : ""}`
            );
            // Rétrocompat : afficher l'ancienne livraison si aucune en DB
            const legacyNote = deliveries.length === 0 && item.deliveryType
              ? `\n\n⚠️ _Ancien système : ${emojiMap[item.deliveryType] ?? item.deliveryType}. Ajoutez un élément pour migrer._`
              : "";
            const msgText = deliveries.length > 0
              ? `📦 *Livraisons de "${item.name}"* — ${deliveries.length} élément(s)\n\n${dlvLines.join("\n")}${legacyNote}`
              : `📦 *Livraisons de "${item.name}"*\n\n_Aucune livraison automatique configurée._${legacyNote}`;
            const delButtons: TelegramBot.InlineKeyboardButton[][] = deliveries.map((d, i) =>
              [{ text: `🗑️ Supprimer #${i + 1} (${emojiMap[d.type] ?? d.type})`, callback_data: `bqa_dlvdel_${d.id}_${itemId}` }]
            );
            await sendMenu(chatId, msgText, { inline_keyboard: [
              ...delButtons,
              [{ text: "➕ Ajouter un élément", callback_data: `bqa_dlvadd_${itemId}` }],
              ...(deliveries.length > 0 ? [[{ text: "🗑️ Tout supprimer", callback_data: `bqa_dlvdelall_${itemId}` }]] : []),
              [{ text: "← Retour à l'article", callback_data: `bqa_item_${itemId}` }],
            ]});
            return;
          }
          ...
          if (/^bqa_dlvadd_\d+$/.test(data) && isAdmin(userId)) {
            ...
            if (!item) return;
            await sendMenu(chatId,
              `➕ *Ajouter un élément à "${item.name}"*\n\nQuel type d'élément souhaitez-vous ajouter ?`,
          ...
          if (data.startsWith("bqa_dlvadd_text_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "📝 Envoie maintenant le *message texte* à livrer.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          if (data.startsWith("bqa_dlvadd_photo_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "🖼️ Envoie maintenant la *photo* à livrer (légende optionnelle).", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          if (data.startsWith("bqa_dlvadd_video_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "🎥 Envoie maintenant la *vidéo* à livrer.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          if (data.startsWith("bqa_dlvadd_doc_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "📄 Envoie maintenant le *document* à livrer.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          if (data.startsWith("bqa_dlvadd_audio_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "🎵 Envoie maintenant le *fichier audio* à livrer.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          if (data.startsWith("bqa_dlvadd_anim_") && isAdmin(userId)) {
            ...
            await bot.sendMessage(chatId, "🎞️ Envoie maintenant le *GIF/animation* à livrer.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "❌ Annuler", callback_data: `bqa_edit_delivery_${itemId}` }]] } });
            return;
          }
          ...
          if (data.startsWith("bqa_dlvdel_") && isAdmin(userId)) {
            ...
            const { item, deliveries } = await getItemWithDeliveries(itemId);
            if (!item) return;
            const emojiMap2: Record<string, string> = { text: "📝", photo: "🖼️", video: "🎥", document: "📄", audio: "🎵", animation: "🎞️" };
            const dlvLines2 = deliveries.map((d, i) => `${i + 1}. ${emojiMap2[d.type] ?? "❓"} *${d.type}*${d.content ? ` — "${d.content.slice(0, 40)}"` : ""}`);
            const msgText2 = deliveries.length > 0
              ? `📦 *Livraisons de "${item.name}"* — ${deliveries.length} élément(s)\n\n${dlvLines2.join("\n")}`
              : `📦 *Livraisons de "${item.name}"*\n\n_Aucune livraison automatique configurée._`;
            const delButtons2: TelegramBot.InlineKeyboardButton[][] = deliveries.map((d, i) =>
              [{ text: `🗑️ Supprimer #${i + 1} (${emojiMap2[d.type] ?? d.type})`, callback_data: `bqa_dlvdel_${d.id}_${itemId}` }]
            );
          ...
          if (data.startsWith("bqa_dlvdelall_") && isAdmin(userId)) {
            ...
            const item = await getItemById(itemId);
            await sendMenu(chatId,
              `✅ Toutes les livraisons supprimées pour *${item?.name ?? "l'article"}*.`,
              { inline_keyboard: [
                [{ text: "➕ Ajouter un élément", callback_data: `bqa_dlvadd_${itemId}` }],
                [{ text: "← Retour à l'article", callback_data: `bqa_item_${itemId}` }],
              ]}
        ...
        // ── Boutique Admin : finaliser la création (multi-livraison) ───────
        if (data.startsWith("bqa_dlv_done_") && isAdmin(userId)) {
          ...
          const { name, desc, price, photoFileIds, deliveries } = bqState;
          pendingBqaAdmin.delete(userId);
          ...
          const cat = await getCategoryById(catId);
          const items = await getItemsByCategory(catId);
          await sendMenu(chatId, `✅ *Article "${item.name}" créé !* (${deliveries.length} élément(s) de livraison)\n\n📁 *${cat?.name ?? "Dossier"}* — ${items.length} article(s)`, bqaCatKeyboard(items, catId, cat?.parent ?? "formations"));
          return;
        }

        // ── Boutique Admin : ajouter un autre élément à la création ─────────
        if (data.startsWith("bqa_dlv_more_") && isAdmin(userId)) {
          ...
          if (!bqState || bqState.step !== "item_delivery") return;
          await bot.sendMessage(chatId,
            `➕ *Ajouter un autre élément* (${bqState.deliveries.length} déjà ajouté(s))\n\nQuel type d'élément souhaitez-vous ajouter ?`,
            { parse_mode: "Markdown", reply_markup: deliveryTypeKb(`bqa_cat_${catId}`) }
          );
          return;
        }
        ...
        if (data.startsWith("bqa_dlv_") && isAdmin(userId)) {
          ...
          if (data === "bqa_dlv_skip") {
            ...
            const cat = await getCategoryById(catId);
            const items = await getItemsByCategory(catId);
            const suffix = accumulated.length > 0 ? `${accumulated.length} élément(s) de livraison` : "livraison manuelle";
            await sendMenu(chatId, `✅ *Article "${item.name}" créé !* (${suffix})\n\n📁 *${cat?.name ?? "Dossier"}* — ${items.length} article(s)`, bqaCatKeyboard(items, catId, cat?.parent ?? "formations"));
            return;
          }
  ...
  bot.on("message", async (msg) => {
    ...
    if (isAdmin(senderUserId) && msg.photo && msg.photo.length > 0) {
      ...
      if (pendingBqaAdmin.has(senderUserId)) {
        ...
        if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "photo") {
          const { catId, name, desc, price, photoFileIds, deliveries } = bqState;
          const newDeliveries = [...deliveries, { type: "photo", fileId: photoFileId, content: caption }];
          pendingBqaAdmin.set(senderUserId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
          await bot.sendMessage(senderChatId,
            `✅ *Élément 🖼️ Photo ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine la création.`,
      ...
      if (pendingBqaEdit.has(senderUserId)) {
        ...
        if (editState.step === "delivery_add_content" && editState.deliveryType === "photo") {
          ...
          const item = await getItemById(editState.itemId);
          await bot.sendMessage(senderChatId, `✅ Élément 🖼️ Photo ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
          return;
        }
      }
    }
    ...
    if (isAdmin(senderUserId) && msg.video) {
      ...
      if (pendingBqaAdmin.has(senderUserId)) {
        ...
        if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "video") {
          ...
          pendingBqaAdmin.set(senderUserId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
          await bot.sendMessage(senderChatId,
            `✅ *Élément 🎥 Vidéo ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine.`,
            { parse_mode: "Markdown", reply_markup: deliveryMoreKb(catId, newDeliveries.length, `bqa_cat_${catId}`) }
          );
          return;
        }
      ...
      if (pendingBqaEdit.has(senderUserId)) {
        ...
        if (editState.step === "delivery_add_content" && editState.deliveryType === "video") {
          ...
          const item = await getItemById(editState.itemId);
          await bot.sendMessage(senderChatId, `✅ Élément 🎥 Vidéo ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
          return;
        }
      }
    }
    ...
    if (isAdmin(senderUserId) && msg.document) {
      ...
      if (pendingBqaAdmin.has(senderUserId)) {
        ...
        if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "document") {
          ...
          pendingBqaAdmin.set(senderUserId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
          await bot.sendMessage(senderChatId,
            `✅ *Élément 📄 Document ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine.`,
            { parse_mode: "Markdown", reply_markup: deliveryMoreKb(catId, newDeliveries.length, `bqa_cat_${catId}`) }
          );
          return;
        }
      ...
      if (pendingBqaEdit.has(senderUserId)) {
        ...
        if (editState.step === "delivery_add_content" && editState.deliveryType === "document") {
          ...
          const item = await getItemById(editState.itemId);
          await bot.sendMessage(senderChatId, `✅ Élément 📄 Document ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
          return;
        }
      }
    }
    ...
    if (isAdmin(senderUserId) && msg.audio) {
      ...
      if (pendingBqaAdmin.has(senderUserId)) {
        ...
        if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "audio") {
          ...
          pendingBqaAdmin.set(senderUserId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
          await bot.sendMessage(senderChatId,
            `✅ *Élément 🎵 Audio ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine.`,
            { parse_mode: "Markdown", reply_markup: deliveryMoreKb(catId, newDeliveries.length, `bqa_cat_${catId}`) }
          );
          return;
        }
      ...
      if (pendingBqaEdit.has(senderUserId)) {
        ...
        if (editState.step === "delivery_add_content" && editState.deliveryType === "audio") {
          ...
          const item = await getItemById(editState.itemId);
          await bot.sendMessage(senderChatId, `✅ Élément 🎵 Audio ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
          return;
        }
      }
    }
    ...
    if (isAdmin(senderUserId) && msg.animation) {
      ...
      if (pendingBqaAdmin.has(senderUserId)) {
        ...
        if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "animation") {
          ...
          pendingBqaAdmin.set(senderUserId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
          await bot.sendMessage(senderChatId,
            `✅ *Élément 🎞️ GIF ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine.`,
            { parse_mode: "Markdown", reply_markup: deliveryMoreKb(catId, newDeliveries.length, `bqa_cat_${catId}`) }
          );
          return;
        }
      ...
      if (pendingBqaEdit.has(senderUserId)) {
        ...
        if (editState.step === "delivery_add_content" && editState.deliveryType === "animation") {
          ...
          const item = await getItemById(editState.itemId);
          await bot.sendMessage(senderChatId, `✅ Élément 🎞️ GIF ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
          return;
        }
      }
    }
    ...
    if (isAdmin(userId) && pendingBqaAdmin.has(userId)) {
      ...
      if (bqState.step === "item_delivery_content" && bqState.currentDeliveryType === "text") {
        const { catId, name, desc, price, photoFileIds, deliveries } = bqState;
        const newDeliveries = [...deliveries, { type: "text", fileId: null, content: text.trim() }];
        pendingBqaAdmin.set(userId, { step: "item_delivery", catId, name, desc, price, photoFileIds, deliveries: newDeliveries });
        await bot.sendMessage(chatId,
          `✅ *Élément 📝 Texte ajouté !* (${newDeliveries.length} élément(s) au total)\n\nAjoute un autre élément ou termine la création.`,
          { parse_mode: "Markdown", reply_markup: deliveryMoreKb(catId, newDeliveries.length, `bqa_cat_${catId}`) }
        );
        return;
      }
    ...
    if (isAdmin(userId) && pendingBqaEdit.has(userId) && !text.startsWith("/")) {
      ...
      if (editState.step === "delivery_add_content" && editState.deliveryType === "text") {
        pendingBqaEdit.delete(userId);
        await createDelivery({ itemId: editState.itemId, type: "text", fileId: null, content: text.trim() });
        const item = await getItemById(editState.itemId);
        await bot.sendMessage(chatId, `✅ Élément 📝 Texte ajouté à *${item?.name}*.`, { parse_mode: "Markdown", reply_markup: bqaItemDetailKeyboard(editState.itemId, editState.catId) });
        return;
      }
      return;
    }
